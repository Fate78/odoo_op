...
class PostWorkPackages(models.AbstractModel):
    ...
    def post_work_package(self, project_id, responsible_id, main_url, name, description, start_date):
        ...
        response = env.post_response(main_url,
                                     env.get_payload(project_id, responsible_id, name, description, start_date))
                            ...
                            while next_offset_wp:
                                for rw in response_work_package['_embedded']['elements']:
                                    ...
                                    if hashed_op == hashed_new:
                                        ...
                                        task_exists = False
                                next_offset_wp, response_work_package = env_wp.check_next_offset(response_work_package)

                        next_offset_member, response_members = env_wp.check_next_offset(response_members)
            if task_exists is False and is_admin is True:
                post_work_package = env_wp.post_response(project_wp_url,
                                                         env_wp.get_payload(project_id, admin_id, wp_name, description,
                                                                            start_date))
